...
class MongoInstance extends events_1.EventEmitter {
    ...
    checkErrorInLine(line) {
        ...
        if (/exception in initAndListen: \w+[^:]: .+[^,], terminating/i.test(line)) {
            ...
            this.emit(MongoInstanceEvents.instanceError, new errors_1.StdoutInstanceError(`Instance Failed to start with "${errorName}". Original Error:\n` + origError));
        }
        ...
        if (/lib[^:]+(?=: cannot open shared object)/i.test(line)) {
            const lib = (_b = (_a = line.match(/(lib[^:]+)(?=: cannot open shared object)/i)) === null || _a === void 0 ? void 0 : _a[1].toLocaleLowerCase()) !== null && _b !== void 0 ? _b : 'unknown';
