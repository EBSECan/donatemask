...
const MessageRoll = () => {
	...
	useEffect(() => {
		...
		axios.get("https://donatemask.ca/api/get_mask_requests").then((res) => {
...
const Summary = () => {
	...
	const [totalTestsFulfilled, setTotalTestsFulfilled] = useState(0);
	const [totalTestsRequested, setTotalTestsRequested] = useState(0);
	...
	useEffect(() => {
		...
		axios.get("https://donatemask.ca/api/get_mask_requests").then((res) => {
			...
			getTotalTests(res.data, "requests");
		});
	}, []);

	const getTotalTests = (data, type) => {
		// Iterating through donations, determining total number of masks donated.
		var count = 0;
		let fulfilledTestCount = 0;
		for (let i = 0; i < data.length; i++) {
			if (type == "requests" && data[i].testAmnt) {
				count += data[i].testAmnt;

				// count fulfilledMask
				if (data[i].requestFulfilled && data[i].requestFulfilled == true) {
					fulfilledTestCount += data[i].testAmnt;
				}
			} else if (data[i].testAmnt) {
				count += data[i].testAmnt;
			}
		}

		setTotalTestsRequested(count);
		setTotalTestsFulfilled(fulfilledTestCount);
	};
								...
								<Col lg="3">
									<Card className="card-lift--hover shadow border-0">
										<CardBody className="py-5">
											...
											<Button className="mt-4" color="success" href="/request">
												Request
											</Button>
										</CardBody>
									</Card>
								...
								<Col lg="3">
									<Card className="card-lift--hover shadow border-0">
										<CardBody className="py-5">
											...
											<Button className="mt-4" color="warning" href="/donate">
												Donate
											</Button>
										</CardBody>
									</Card>
